  ...
  def input(%{type: "select"} = assigns) do
    ...
    <div phx-feedback-for={@name}>
      ...
      <select
        id={@id}
        name={@name}
        class="mt-1 block w-full rounded-md border border-gray-300 bg-white shadow-sm focus:border-slate-400 focus:ring-0 sm:text-sm"
        multiple={@multiple}
        {@rest}
      >
    ...
    <div phx-feedback-for={@name}>
      ...
      <textarea
        ...
        class={[
          ...
          "min-h-[6rem] border-slate-300 focus:border-slate-400",
          ...
          <tr :for={row <- @rows} id={@row_id && @row_id.(row)} class="group hover:bg-slate-50">
            ...
            >
              <div class="block py-4 pr-6">
                <span class={"absolute -inset-y-px right-0 -left-4 group-hover:bg-slate-50 sm:rounded-l-xl #{@row_class && @row_class.(row)}"} />
