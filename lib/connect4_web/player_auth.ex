...
defmodule Connect4Web.PlayerAuth do
  @moduledoc """
  Authentication gubbins, generated by `mix phx.gen.auth`.
  """
  ...
  def fetch_current_player(conn, _opts) do
    ...
    player = player_token && Auth.get_player_by_session_token(player_token)
  ...
  the current_player:
      ...
      defmodule Connect4Web.PageLive do
        use Connect4Web, :live_view

        on_mount {Connect4Web.PlayerAuth, :mount_current_player}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{Connect4Web.PlayerAuth, :ensure_authenticated}] do
  ...
  def on_mount(:ensure_authenticated, _params, session, socket) do
    ...
    if socket.assigns.current_player do
      ...
      socket =
        ...
        |> LiveView.redirect(to: ~p"/players/log_in")
  ...
  defp mount_current_player(session, socket) do
    Component.assign_new(socket, :current_player, fn ->
      if player_token = session["player_token"] do
        Auth.get_player_by_session_token(player_token)
      end
    end)
  end
  ...
  defp put_token_in_session(conn, token) do
    ...
    |> put_session(:live_socket_id, "players_sessions:#{encoded_token}")
  end
