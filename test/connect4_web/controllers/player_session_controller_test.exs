...
defmodule Connect4Web.PlayerSessionControllerTest do
  ...
  describe "POST /players/log_in" do
    ...
    test "logs the player in with remember me", %{conn: conn, player: player} do
      ...
      assert conn.resp_cookies["_connect4_web_player_remember_me"]
      assert redirected_to(conn) == ~p"/"
    end
    ...
    test "login following registration", %{conn: conn, player: player} do
      ...
      assert redirected_to(conn) == ~p"/"
      assert Flash.get(conn.assigns.flash, :info) =~ "Account created successfully"
    end
    ...
    test "login following password update", %{conn: conn, player: player} do
      ...
      assert Flash.get(conn.assigns.flash, :info) =~ "Password updated successfully"
    end
